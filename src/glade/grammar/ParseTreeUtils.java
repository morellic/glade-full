...
public class ParseTreeUtils {
	...
	public static class ParseTreeConstantNode implements ParseTreeNode {
		private final ConstantNode node;
		private final String example;
		
		public ParseTreeConstantNode(ConstantNode node, String example) {
			this.node = node;
	...
	public static ParseTreeNode getParseTreeRepConst(Node node) {
		if(node instanceof RepetitionNode) {
			...
			return new ParseTreeConstantNode((ConstantNode)node, node.getData().example);
		} else {
