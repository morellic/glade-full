...
public class GrammarSerializer {
	...
	public static void serialize(Node root, NodeMerges merges, DataOutputStream dos) throws IOException {
		List<Node> nodes = GrammarUtils.getAllNodes(root);
		...
		dos.writeInt(merges.keySet().size()); // 4
		for(Node first : merges.keySet()) {
			dos.writeInt(merges.get(first).size()); // 5
			for(Node second : merges.get(first)) {
	...
	public static Pair<Node,NodeMerges> deserializeNodeWithMerges(DataInputStream dis) throws IOException {
		...
		return new Pair<Node,NodeMerges>(nodes.get(0), merges);
	}
}
