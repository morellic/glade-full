...
public class GrammarTransformer {
	public static Node getTransform(Node node, DiscriminativeOracle oracle, boolean useConstantTransformer, boolean useCharacterClasses) {
		Node transformFlatten = getTransform(node, new FlattenTransformer());
		return getTransform(transformFlatten, new ConstantTransformer(oracle, getMultiAlternationRepetitionConstantNodes(transformFlatten), useConstantTransformer, useCharacterClasses));
	}
	...
	private static MultiConstantNode generalizeConstant(ConstantNode node, DiscriminativeOracle oracle, boolean useConstantTransformer, boolean useCharacterClasses) {
		...
		for(int i=0; i<example.length(); i++) {
			...
			if(useConstantTransformer && useCharacterClasses) {
			...
			} else if(useConstantTransformer) {
				for(char c : CharacterUtils.getAsciiCharacters()) {
					if(curC != c) {
						List<String> checks = Utils.getList("" + c);
						if(GrammarSynthesis.getCheck(oracle, curContext, checks)) {
							characterOption.add(c);
							characterCheck.add(c);
						}
	...
	private static MultiAlternationNode generalizeMultiAlternationConstant(MultiAlternationNode node, MultivalueMap<MultiAlternationNode,ConstantNode> multiAlternationNodeConstantChildren, DiscriminativeOracle oracle, boolean useConstantTransformer, boolean useCharacterClasses) {
		...
		for(Node child : multiAlternationNodeConstantChildren.get(node)) {
			if(!isContained(child.getData().example, curConsts)) {
				curConsts.add(generalizeConstant((ConstantNode)child, oracle, useConstantTransformer, useCharacterClasses));
			}
		}
	...
	private static class ConstantTransformer implements NodeTransformer {
		...
		private final boolean useConstantTransformer;
		private final boolean useCharacterClasses;
		private ConstantTransformer(DiscriminativeOracle oracle, MultivalueMap<MultiAlternationNode,ConstantNode> multiAlternationNodeConstantChildren, boolean useConstantTransformer, boolean useCharacterClasses) {
			this.oracle = oracle;
			...
			this.useConstantTransformer = useConstantTransformer;
			this.useCharacterClasses = useCharacterClasses;
		}
		public Node transformConstant(ConstantNode node) {
			return this.ignoredConstants.contains(node) ? node : generalizeConstant(node, this.oracle, this.useConstantTransformer, this.useCharacterClasses);
		}
		...
		public Node transformMultiAlternation(MultiAlternationNode node, List<Node> newChildren) {
			return this.multiAlternationNodeConstantChildren.containsKey(node) ? generalizeMultiAlternationConstant(node, this.multiAlternationNodeConstantChildren, this.oracle, this.useConstantTransformer, this.useCharacterClasses) : new MultiAlternationNode(node.getData(), newChildren);
		}
